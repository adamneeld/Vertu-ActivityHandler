...
//#handles routing via sip domain
const handleSipDomain = async (payload, voiceClient) => {
    const sipDomain = "62.252.237.190";
    if(payload.destination.startsWith("8")) {
        payload.destination = `sip:${"1111".concat(payload.destination)}@${sipDomain};edge=${voiceClient._edge ? voiceClient._edge : "dublin"}`;
    }
    else if (payload.destination.startsWith("1111")){
        payload.destination = `sip:${payload.destination}@${sipDomain};edge=${voiceClient._edge ? voiceClient._edge : "dublin"}`;
    }
}
//# endhandles routing via sip domain
...
Actions.addListener('beforeStartOutboundCall', async (payload, abortFunction) => {
  ...
  // and hearing ring tone before the reservation is accepted. Also, we can't
	...
	// call is initiated.
	const { voiceClient, workerClient } = Manager.getInstance();
	const { internationalDial } = workerClient.attributes || false;
	...
	// if internationalDial === true, allow calls to international numbers
	if(internationalDial) {
		handleSipDomain(payload, voiceClient);
		setWorkerActivity(targetActivity?.sid);
		await WorkerState.waitForWorkerActivityChange(targetActivity?.sid);
	
	}
	else {
		// If destination is UK number or Cisco contact. Blocks calling to international numbers
		if(payload.destination.startsWith("0") || payload.destination.startsWith("+44") || payload.destination.startsWith("8") || payload.destination.startsWith("1111")){
			handleSipDomain(payload, voiceClient);
			setWorkerActivity(targetActivity?.sid);
			await WorkerState.waitForWorkerActivityChange(targetActivity?.sid);
		}
